...
export const commitOptions =(options)=>{
    switch(options){
        case "-a":
        case '--all':
        ...
        case `-C`:
        case `-C <commit>`:
        case `--reuse-message=<commit>`:
            `Take an existing commit object, and reuse the log message and the authorship information (including the timestamp) when creating the commit.`
            break;

        case `-c <commit>`:
        case `--reedit-message=<commit>`:
            `Like -C, but with -c the editor is invoked, so that the user can further edit the commit message.`
            break;

        case `--fixup=[(amend|reword):]<commit>`:
        ...
        case `<pathspec>…​`:
            ...
            break;
        default: "this is not an option command for git-commit"
    }
}